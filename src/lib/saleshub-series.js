...
import { bucketSeries, scaleSeriesToTotal } from "./portfolio-series";
 ...
 *
 * Totals come from the client groups' own call stats rather than from these
 * rows, because the rows endpoint caps what it returns: a curve built straight
 * from a capped sample undercounts and would sit beneath a headline figure that
 * doesn't. `scaleSeriesToTotal` keeps the shape the sample showed and takes the
 * magnitude from the uncapped total, and marks the series estimated when it
 * had to — the chart says so rather than letting a sample read as a census.
 *
 * @param {object[]} leadRows rows from /api/hotprospector/call-center
 * @param {boolean} capped whether that fetch hit its row limit
 * @param {{calls: number, called: number, inbound: number, talk: number}} totals
 * @param {string} granularity Daily | Weekly | Monthly
 */
export function buildSalesSeries(leadRows, capped, totals, granularity) {
  ...
  const on = (rows, getDate, weight) => alignTo(axis, bucketSeries(rows, getDate, granularity, weight));
  const scale = (series, total) => scaleSeriesToTotal(series, total, capped);

  return {
    calls: scale(axis, totals.calls),
    called: scale(on(firstCallPerLead(leadRows), (l) => l.at), totals.called),
    inbound: scale(on(inbound, (l) => l.at), totals.inbound),
    talk: scale(on(logs, (l) => l.at, (l) => l.minutes), totals.talk),
  };
}
