...
export default function ClientsPage() {
  ...
  // ===== DYNAMIC COLUMNS BASED ON CLIENT GROUPS DATA =====
  const columns = useMemo(() => {
    console.log("🔄 Building dynamic columns from client groups:", clientGroups.length);
    
    // Build dynamic columns including discovered tags
    const dynamicColumns = buildDynamicColumns(clientGroups);
    console.log("📊 Dynamic columns built:", dynamicColumns.length);
    
    // Add custom metrics from localStorage
    ...
    const unique = all.filter((col) => {
      if (seen.has(col.id)) return false;
      seen.add(col.id);
      return true;
    });
    
    console.log("✅ Final columns count:", unique.length);
    
    // Log discovered tags
    const tagColumns = unique.filter(col => col.category === 'tags');
    if (tagColumns.length > 0) {
      console.log("🏷️ Discovered tags:", tagColumns.map(c => c.tagName));
    }
  ...
  const [columnVisibility, setColumnVisibility] = useState(() => {
    const map = {};
    BASE_CLIENT_COLUMNS.forEach((c) => (map[c.id] = c.visible));
    return map;
  });

  // Update column visibility when new columns are discovered
  useEffect(() => {
    setColumnVisibility(prev => {
      const updated = { ...prev };
      columns.forEach(col => {
        if (!(col.id in updated)) {
          // New column discovered - set to visible default
          updated[col.id] = col.visible ?? false;
        }
      });
      return updated;
    });
  ...
  const categories = ENHANCED_CATEGORIES;
  ...
  const save = () => {
    localStorage.setItem('clientsPage-columnVisibility', JSON.stringify(columnVisibility));
    setIsOpen(false); 
  ...
  const fetchClientGroups = async (forceRefresh = false) => {
    try {
      ...
      const uniqueTags = extractUniqueTags(groups);
      console.log("🏷️ Discovered unique tags:", uniqueTags);
