...
export default function Home() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      // Fetch clients data
      const clientsRes = await fetch("http://localhost:5000/api/get_all_clients", {
        credentials: "include",
      })
      ...
      // Fetch GHL locations
      const locationsRes = await fetch("http://localhost:5000/api/location-data", {
        credentials: "include",
      })
      const locationsData = locationsRes.ok ? await locationsRes.json() : { locations: [] }

      // Fetch Meta ad accounts
      const adAccountsRes = await fetch("http://localhost:5000/api/facebook/adaccounts", {
        credentials: "include",
      })
