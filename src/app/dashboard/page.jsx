...
function AlertSkeletonItem() {
  return (
    <div className="flex items-center gap-4 mb-3 p-4 rounded-lg border-l-4 border-l-muted bg-white shadow-sm">
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-1.5 bg-gray-50 border border-border/60 rounded-full px-2.5 py-1 shrink-0">
          ...
          <span className="text-xs font-medium text-foreground">{isPending ? "Syncing..." : "Healthy"}</span>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2">
        {[{ label: "Ad Spend", value: ad_spend }, { label: "CTR", value: ctr }, { label: "Clicks", value: clicks }].map(({ label, value }) => (
          <div key={label}>
      ...
      <div>
        ...
        <div className="mt-4 bg-[#F3F0FD] rounded-2xl p-5">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 max-w-2xl mx-auto">
            ...
            {["Why are bookings down this week?", "Which tag gave the best cost per booking?", "Show ROAS breakdown by campaign"].map((s) => (
              <button
                key={s}
                onClick={() => setQuery(s)}
